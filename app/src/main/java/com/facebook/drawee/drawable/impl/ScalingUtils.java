...
public class ScalingUtils {
    ...
    /**
     * 选择扩展孩子的界限到父界限。
     * 类似于{@link android.widget.ImageView.ScaleType}，但是ScaleType.MATRIX 部支持
     * 要使用matrix扩展，使用{@link MatrixDrawable}，一个额外的scale(FOCUS_CROP)被提供
     */
    public interface ScaleType {

        /**
         * 尺度的宽度和高度独立,所以孩子父完全匹配。这可能改变孩子的长宽比.
         */
        ...
        /**
         * 获取基于scale type的transformation matrix
         * @param outTransform 输出矩阵来储存结果
    ...
    /**
     * Scaletypes有一些内部状态,不是静态的
     */
    public interface StatefulScaleType {

        /**
         * 返回内部状态，返回的对象必须是不变的!
         * 返回的状态可能使用{@code ScaleType.getTransform}.结果的缓存
         * 如果是null，结果不会被缓存，如果不是null，旧的转换可以使用如果是一个相同的状态
         */
        public Object getState();
    }

    /**
     * 在两个scale types中间的插值态的Scale type
     */
    public static class InterpolatingScaleType implements ScaleType, StatefulScaleType {
         ...
         * 设置插值数量
         *
         * 0.0就是完全和ScaleTypeFrom相同
         * 1.0就是完全和ScaleTypeTo相同.
         * 中间画值将产生一个变换两者之间的线性组合。.
         */
