...
import com.facebook.commom.internal.Sets;
...
/**
 * 一个基本的pool类，该类管理一个 V type 的pool
 * 这个pool被组织成一个map，每一个entry在map中是一个free-list(一个队列)，这个队列的大小是entry被给予的大小
 * 有一些pool有固定的buckets(又名桶的大小)，而有一些是不固定的
 *
 * 这个pool主要支持两种操作：
 * 1.{@link #get(int)} - 返回一个大小与size相同或者超过的V，这个V希望从池子中分配，如果池子没有空了，只能自己去分配
 * 2.{@link #release(V)} - 将这个资源释放回pool中
 * 此外，这个pool订阅了{@link MemoryTrimmableRegistry}，会返回一个内存较低的事件，此时有一些
 * V(可能是所有)将会被释放(通过底层的方法，我们不用管)，这时此部分内存不再属于pool
 *
...
public abstract class BasePool<V> implements Pool<V> {
    ...
    /**
     * 这是一个稀疏数组，下标是V需要分配的内存大小，其内部是有序的所以可以通过二分查找来找到某个Bucket。
    ...
    /**
     * 一个Identity hash-set，用来保持追踪已经被使用的V，这个Set是以引用相等为区分
    ...
    /**
     * 从Pool中获取一个V，如果Pool已经没有空间，那么自己Allocates一个V。
    ...
    public V get(int size) {
        ...
        synchronized (this) {
            ...
            //这里是桶中已经没有多余的空V了，这里在检查我们是否还能去内存中分配一个我们需要的V
        ...
        //如果我们的桶中已经没有空V，内存又允许我们分配一个我们需要的V
        V value = null;
        try {
            //那么通过调用子类的具体分配内存的方法，如分配一个Bitmap
            //这里在同步块外部进行内存分配，虽然我们可以在内部分配，但是这样就不能并发的操作Pool了
    ...
    /**
     * 将一个V释放到Pool中。
     * 在一些情况下，会将该V直接释放，而不是释放到池子中
     * 1.如果目前的Pool内存已经超过了最大值
     * 2.如果这个V已经不能映射到bucket中了
     * 3.如果bucket的长度已经超过了最大长度
     * 4.如果V不需要被Pool组织
    ...
    /**
     * 当内存过低的时候会触发这个方法，需要子类去实现，以清除一部分Pool的内存或者全部
