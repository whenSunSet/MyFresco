...
public class CountingMemoryCache<K, V> implements MemoryCache<K, V>, MemoryTrimmable {
    ...
    public CloseableReference<V> cache(
        ...
        synchronized (this) {
            ...
            //如果之前 全部条目Map 中已经有了这个键值对 那么将其设置为孤独的并且获取其value引用
            if (oldEntry != null) {
                makeOrphan(oldEntry);
    ...
    public CloseableReference<V> get(final K key) {
        ...
        synchronized (this) {
            ...
            //如果从 全部条目Map 中获取的条目不为空 那么新建一个 客户端引用
            if (entry != null) {
