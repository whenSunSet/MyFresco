...
/**
 * 指定在image pipeline中使用的网络获取者的接口
 * Interface that specifies network fetcher used by the image pipeline.
 *
 * 强烈建议实现一个使用{@link Executor}在她们{@link #fetch}方法中在不同的线程发起请求
 ...
 *
 * 当从网络获取失败或者请求取消，子类有责任调用{@link Callback}中的方法，如果没有调用，
 * 那么pipeline将不会知道image的获取是失败的，那么app可能失去控制
...
public interface NetworkFetcher<FETCH_STATE extends FetchState> {
    ...
    /**
     * 创建一个新的实例继承于{@link FetchState}派生的对象用于存储状态。
