...
/**
 * Abstract Drawee controller用来获取image
...
public abstract class AbstractDraweeController<T, INFO> implements
    ...
    /**
     * 这个class被使用于最优化的创建一个ForwardingControllerListener
     * 当只要一个controller listener的时候
    ...
    /**
     * 初始化各种成员，在构造器中调用。
     * 这里允许重新使用已经存在的controller代替新的
     * 这个犯法应该在controller detached 的时候调用
    ...
    //执行释放 操作，将RManager、GDetector 、SDraweeHierarchy reset，调用releaseFetch
    @Override
    public void release() {
    ...
    //关闭DataSource资源，释放mDrawable和mFetchedImage
    ...
    //判断是否有缓存image(默认无)，如果有CListener.onSubmit()、onNewResultInternal()
    // 如果没有getDataSource()、创建BaseDataSubscriber、DataSource.subscribe(BaseDataSubscriber，Executor)
    // (注册观察者，当数据变化的时候调用BaseDataSubscriber的各种实现进行处理)
    ...
    //BaseDataSubscriber中成功的时候被调用，
    // 然后createDrawable()从返回的图片数据中创建Darwable，
    // 在一切正常的情况下会调用SimpleDraweeHierarchy.setImage(Drawable(刚刚创建的))
